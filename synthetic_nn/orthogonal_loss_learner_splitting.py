...
def train_stage2(loader, model_f, optimizer, epochs=5):
    ...
    for ep in range(epochs):
        for X1, X2, Y, T, r1, r2, t_hat, y_hat in loader:
            ...
            err = Y - (T - t_hat)/t_hat*torch.tanh(r1 - r2) - (f1 - f2)*t_hat
            # err = Y - (T - t_hat)/t_hat *y_hat - (f1 - f2)*t_hat
            reward_diff = torch.clamp(torch.abs(r1 - r2), min=1e-5)
            # err = Y - (T* reward_diff - torch.tanh(reward_diff)) - (f1 - f2)*torch.tanh(reward_diff)/(reward_diff)
            loss = (err**2).mean()
