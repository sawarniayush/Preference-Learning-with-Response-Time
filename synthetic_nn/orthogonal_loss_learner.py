...
def train_stage1(loader, model_r, model_t, model_y, opt_r, opt_t, opt_y, bce, mse, a=1.0, epochs=5):
...
def train_stage2(loader, model_f, optimizer, a=1.0, epochs=5):
    model_f.train()
    for ep in range(epochs):
        for X1, X2, Y, T, r1, r2, t_hat, y_hat in loader:
            f1 = model_f(X1); f2 = model_f(X2)
            err = Y - (T - t_hat)*(r1 - r2)/a - (f1 - f2)*t_hat/a
            loss = (err**2).mean()
...
def evaluate_f(loader, model_f):
    ...
    for x1, x2, y, t, true_r1, true_r2, *_ in loader:
        ...
        total += y.size(0)
        mse += (((true_r1 - true_r2) - (f1-f2))**2).sum().item()
...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=1.0,
                        help='Barrier/threshold a for two-stage training')
    ...
    # Stage1 train
    model_r, model_t, model_y = train_stage1(loader1, model_r, model_t, model_y, opt_r, opt_t, opt_y, bce, mse, a=args.threshold, epochs=args.epochs1)
    ...
    model_f = train_stage2(loader3, model_f, opt_f, a=args.threshold, epochs=args.epochs2)

    # Evaluate on test set
    df_test = pd.read_pickle(args.test)
    test_ds = TwoStageDataset(df_test, stage='stage1')
    test_loader = DataLoader(test_ds, batch_size=args.batch)
    acc, mse, regret = evaluate_f(test_loader, model_f)
